...
###read in the tweets (they're from Venezuelan senators)
setwd("~/Documents/Text_as_Data/")
...
### the # of docs. to perform the topic model is finely tuned b/c unsupervised
### learning techniques are computationally intensive. 

###Run the model with the same number of topics as before

fitSpec50 <- stm(out_20$documents,out_20$vocab,K=50, init.type="LDA", 
                    content=~team, prevalence = ~team + as.numeric(days))
